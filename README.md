...
> 注意：macOS Intel 用户需修改 electron-builder.json 中的 mac.target.arch 为 ["x64"]。
> 
🚀 核心概念说明
代理模式
 * 智能模式 (推荐)：自动分流，国内直连，国外代理。
 * 全局模式：所有流量强行经过代理服务器。
 * 直连模式：跳过代理，直接连接。
关于 TUN 模式
默认开启 TUN 模式，该模式会接管系统网卡层级的流量。这对于终端（Terminal）、游戏以及不支持系统代理设置的软件非常友好。如果你更倾向于传统的系统代理，可以在设置中手动切换。
🔧 技术栈
 * Core: sing-box
...
📄 协议与声明
 * 开源协议: 本项目基于 MIT 协议。
 * 免责声明: 本软件仅供学习和研究使用，请遵守当地法律法规。使用本软件所产生的任何后果由使用者自行承担。
有任何问题或建议？ 欢迎提交 Issues 或加入我们的 Telegram 频道。