...
describe('§15 doTestAllServers 分流 + testServersViaMainCore 编排', () => {
  it('主核可用 → 池路径：波内 node[k] 用 poolPort[k]、波间串行、不触临时核', async () => {
    ...
    const measureSpy = jest
      .spyOn(svc as unknown as { measureViaTunnel: (p: number) => Promise<unknown> }, 'measureViaTunnel')
      .mockImplementation(async (port: number) => {
        events.push(`meas-${port}`);
        return { latency: port };
      });
    const proxySpy = jest.spyOn(
      svc as unknown as { testServersViaProxy: (...a: unknown[]) => Promise<Map<string, number | null>> },
      'testServersViaProxy'
    );
  ...
  it('主核不可用（available=false）→ 回退临时核 testServersViaProxy，不触 selectSlot', async () => {
    ...
    const svc = new SpeedTestService(mockLog, () => ({}), () => probe);
    const proxySpy = jest
      .spyOn(
        svc as unknown as { testServersViaProxy: (...a: unknown[]) => Promise<Map<string, number | null>> },
        'testServersViaProxy'
...
const tsExit = (id: string): ServerConfig =>
  ({ id, name: id.toUpperCase(), protocol: 'tailscale', tailscaleSettings: { exitNode: '100.64.0.1' } }) as unknown as ServerConfig;
...
describe('§16 波前 gate 缺席（诚实性：绝不写假 -1）+ outcome', () => {
  ...
  it('核中途崩溃（isRunning 翻 false）→ 未测节点缺席、outcome=interrupted', async () => {
    ...
    const svc = new SpeedTestService(mockLog, undefined, () => probe, () => 0);
    jest
      .spyOn(svc as unknown as { measureViaTunnel: (p: number) => Promise<unknown> }, 'measureViaTunnel')
      .mockImplementation(async (port: number) => {
